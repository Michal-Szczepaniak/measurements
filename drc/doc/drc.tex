...
% Pacchetto specifico per supporto HeVeA
\usepackage{hevea}

% Impostazione specifiche per LaTeX PDF
%BEGIN LATEX

\usepackage[pdftitle={DRC: Digital Room Correction},pdfauthor={Denis Sbragion},
pdfstartview=FitH,plainpages=false,pdfpagelabels,
colorlinks=true,bookmarks=true,bookmarksnumbered,linktocpage]{hyperref}
\usepackage{changepage}

% Modifica l'indentazione per l'ambiente quote
...
% Impostazione parametri listing, solo per LaTeX
%BEGIN LATEX
\lstset{
...
%HEVEA \renewcommand{\heveaimagedir}{figures} 

% Definizione stili per HeVeA
\newstyle{BODY}{font-family:Helvetica, Universe, Arial, sans-serif;}
\newstyle{H1}{font-size: x-large; font-weight:bold; color:teal;}
\newstyle{H2}{font-size: large; font-weight:bold; color:teal;}
\newstyle{H3}{font-size: medium; font-weight:bold; color:teal;}
\newstyle{H4}{font-weight:bold; color:teal;}
\newstyle{A}{color:teal;}
...
2019-07-26
\linebreak\linebreak
\small{Copyright \copyright\ 2002-2019 Denis Sbragion}
\linebreak\linebreak
\small{Version 3.2.3}
}
...
\begin{quote}
\href{mailto:d.sbragion@neomerica.it}{d.sbragion@neomerica.it}
\end{quote}
...
% Nuove funzionalità
\section{What's new in version 3.2.3}

A bug in lsconv has been fixed.

\subsection{Compatibility with previous versions}

Configuration files for version 3.2.3 are compatible with the
previous version.

% Modifiche precedenti
\subsection{History: what was new in previous versions}

\subsubsection{Version 3.2.2}
...
limiting. A bug in the estimation of the band limited RMS value has been
...
reasonable estimation of clipping levels, has been added. Many minor
...
\begin{itemize}
 \sloppy\raggedright
 \item \label{RoomEqWizard} Room Eq Wizard \url{http://www.roomeqwizard.com/}
...
\end{itemize}
Since some time there are also some integrated packages providing both
...
\end{itemize}
This packages may be really useful, especially for inexperienced peoples.
...
\end{quote}
which is provided also with individual calibration files.
...
\begin{itemize}
\sloppy\raggedright
 \item BruteFIR: the original BruteFIR site is no longer available.
	A copy of the documentation is available at:
	\begin{quote}
	\sloppy\raggedright
		\url{https://web.archive.org/web/20180521024857/https://www.ludd.ltu.se/~torger/brutefir.html}.
	\end{quote}
	
	A copy of the source code is available in many different places, for example at:
	\begin{quote}
	\sloppy\raggedright
	\url{http://deb.debian.org/debian/pool/main/b/brutefir/brutefir_1.0o.orig.tar.gz}.
	\end{quote}
	Most Linux distributions already include BruteFIR within their standard package set, so there is no need to compile it from source.
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-Linear}
...
is simply windowed out and ignored in the correction.
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-SemiLogY}
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-SemiLogX}
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-LogLog}
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-Gabor}
...
uncertainty in the time-frequency plane. This means for example that,
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-BPComparison}
...
Starting from version 2.7.0 this step is implicitly disabled.
Considering that excess phase inversion is performed simply by time
reversal of the excess phase component, pre-echo is implicitly limited
...
implicitly applied on the left side of the time-frequency plane instead
...
this implies some trade-off on filter accuracy.

\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-Jail}
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-BWidthCmp}
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-SpectralEnvelope}
...
channels, resulting in an obvious channel misalignment if evaluated
...
Recently some further informal tests, based on auralization of standard
music files, have been carried out.
These kind of tests allow for the direct comparison of the original music file with 
the auralized version of it, i.e. of the file processed through the corrected 
room impulse response. The tests need of course to be done through headphones,
so they are not exactly the same as a real versus reproduced comparison,
nevertheless they are quite a good approximation of this kind of comparison.

These tests, done on few different rooms impulse responses, confirmed that
the psychoacoustic target computation based on the spectral envelope provides 
a spectral balance which is really close to being neutral, i.e. a balance which
is really close to the balance of the original unprocessed music file.
...
or sharp attacks. In order to avoid them there are basically two options:
...
(basically homomorphic  deconvolution and pre-echo  truncation inversion),
...
A good estimation of an adequate normalization factor might be provided
...
provide a safe estimation without introducing excessive attenuation.
...
As most audio practitioners already know, in a basic stereo loudspeaker
...
about 10 dB  better than the  signal to noise ratio  provided by the
best DACs available in the world.

Despite these results, considering the computational resources available in modern
computers since version 3.2.2 the supplied Win32 executable is compiled for
double precision. If you want to switch to the single precision you have
...
is important to notice that basically the correction isn't much stronger
...
\begin{figure}
\begin{center}
\includegraphics[width=\basepctwidth\textwidth,keepaspectratio]{DBP-DTFCmp}
...
correction. It should be long enough to accommodate for any subsequent
...
Within this stage the microphone transfer function is inverted and
...
20 Hz - 200 Hz range weights the same as the 2000 Hz - 20000 Hz range.
Setting DLType to P activates the log weighted versions of the linear
phase dip limiting and setting DLType to W activates the log weighted
versions of the minimum phase dip limiting.
...
f and pre-echo truncation is basically disabled because it is already
...
show instead the performance levels achievable combining active room
...
encapsulated postscript files or files in any other format supported by
...
may be easily obtained by supplying some further parameters to the
...
The scripts have been tested with octave 4.4.1 and GnuPlot 5.2.6. Beware
...
Using a different graphics toolkit the results might be different from
those provided in this manual or it might not work at all.
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IRStepResponse}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IRStepResponse}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IRFullRange}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IRFullRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IRFullRangeEnvelope}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IRFullRangeEnvelope}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IRFullRangeETC}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IRFullRangeETC}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IR2kHzBrickwall}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IR2kHzBrickwall}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IR2kHzBrickwallEnvelope}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IR2kHzBrickwallEnvelope}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IR2kHzBrickwallETC}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IR2kHzBrickwallETC}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IR200HzBrickwall}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IR200HzBrickwall}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IR200HzBrickwallEnvelope}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IR200HzBrickwallEnvelope}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-IR200HzBrickwallETC}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-IR200HzBrickwallETC}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRUnsmoothed1ms}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRUnsmoothed1ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRUnsmoothed5ms}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRUnsmoothed5ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRUnsmoothed200ms}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRUnsmoothed200ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRFDWSmoothed}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRFDWSmoothed}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRFDWSmoothed-1-6}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRFDWSmoothed-1-6}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRFDWSmoothed-1-3}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRFDWSmoothed-1-3}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRBarkSmoothed}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRBarkSmoothed}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MRERBSmoothed}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MRERBSmoothed}
...
basically  linear at  least for  the direct  sound,  which implies  also a
constant group delay.

\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-PRUnsmoothed1ms}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-PRUnsmoothed1ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-PRUnsmoothed5ms}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-PRUnsmoothed5ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-PRUnsmoothed200ms}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-PRUnsmoothed200ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-PRFDWSmoothed}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-PRFDWSmoothed}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-PRFDWSmoothed-1-6}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-PRFDWSmoothed-1-6}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-PRFDWSmoothed-1-3}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-PRFDWSmoothed-1-3}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SDHighRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SDHighRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SFHighRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SFHighRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SDMidRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SDMidRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SFMidRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SFMidRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SDBassRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SDBassRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SFBassRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SFBassRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SDHighRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SDHighRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SFHighRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SFHighRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SDMidRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SDMidRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SFMidRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SFMidRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SDBassRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SDBassRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-SFBassRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-SFBassRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-Spectrogram01ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-Spectrogram01ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-Spectrogram20ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-Spectrogram20ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramHTRENVSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramHTRENVSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramHTRENVSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramHTRENVSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramHTRENVMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramHTRENVMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramHTRETCSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramHTRETCSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramHTRETCSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramHTRETCSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramHTRETCMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramHTRETCMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramMTRENVSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramMTRENVSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramMTRENVSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramMTRENVSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramMTRENVMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramMTRENVMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramMTRETCSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramMTRETCSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramMTRETCSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramMTRETCSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-L-MorletScalogramMTRETCMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-R-MorletScalogramMTRETCMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IRStepResponse}
\caption{\label{BaselineStepResponseFullRange} Step response}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IRFullRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IRFullRangeEnvelope}
\caption{\label{BaselineEnvelopeFullRange}  Full  range impulse response
envelope.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IRFullRangeETC}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IR2kHzBrickwall}
\caption{\label{BaselineImpulseResponse2kHzBrickwall}   Impulse response
after brickwall filtering at 2 kHz.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IR2kHzBrickwallEnvelope}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IR2kHzBrickwallETC}
\caption{\label{BaselineETC2kHzBrickwall}    Time-energy  response after
brickwall filtering at 2 kHz.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IR200HzBrickwall}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IR200HzBrickwallEnvelope}
\caption{\label{BaselineEnvelope200HzBrickwall}        Impulse  response
envelope after brickwall filtering at 200 Hz.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-IR200HzBrickwallETC}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRUnsmoothed1ms}
\caption{\label{BaselineMagnitudeUnsmoothed1ms}    Unsmoothed  frequency
response magnitude, 1 ms Blackman window.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRUnsmoothed5ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRUnsmoothed200ms}
\caption{\label{BaselineMagnitudeUnsmoothed200ms}   Unsmoothed frequency
response magnitude, bass range, 200 ms Blackman window.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRFDWSmoothed}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRFDWSmoothed-1-6}
...
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRFDWSmoothed-1-3}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRBarkSmoothed}
...
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MRERBSmoothed}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-PRUnsmoothed1ms}
\caption{\label{BaselinePhaseResponse1}  Unsmoothed phase response, 1 ms
Blackman window.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-PRUnsmoothed5ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-PRUnsmoothed200ms}
\caption{\label{BaselinePhaseResponse200} Unsmoothed phase response, 200
ms Blackman window.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-PRFDWSmoothed}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-PRFDWSmoothed-1-6}
\caption{\label{BaselinePhaseFDWSmoothed16Oct}  Phase response magnitude
smoothed using  a frequency  dependent  windowing providing  a frequency
resolution close to 1/6 of octave smoothing.}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-PRFDWSmoothed-1-3}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SDHighRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SFHighRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SDMidRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SFMidRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SDBassRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SFBassRange}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SDHighRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SFHighRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SDMidRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SFMidRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SDBassRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-SFBassRangeW}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-Spectrogram01ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-Spectrogram20ms}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramHTRENVSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramHTRENVSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramHTRENVMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramHTRETCSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramHTRETCSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramHTRETCMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramMTRENVSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramMTRENVSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramMTRENVMap}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramMTRETCSD}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramMTRETCSF}
...
\begin{figure}
\begin{center}
\includegraphics[width=\pctwidth\textwidth,keepaspectratio]{SR-B-MorletScalogramMTRETCMap}
